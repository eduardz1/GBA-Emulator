...
void Arm7tdmi::LSL_t(Arm7tdmi::_instruction ins)
{
    ...
    set_register(_registers(Rd.word),
                 (static_cast<uint32_t>(Rs.word << (val))));
}
...
void Arm7tdmi::NEG_t(Arm7tdmi::_instruction instruction)
{
    ...
    rn = registers[_registers((instruction.halfword_lo >> 4) & 0xF)]; // rn is
                                                                      // rs in
                                                                      // THUMB
    rd.word = 0 - rn.word;
}
...
void Arm7tdmi::ASR_t(Arm7tdmi::_instruction ins)
{
    ...
    Rs = registers[_registers((ins.halfword_lo >> 3) & 0x7)];
    sign =
        static_cast<uint8_t>(static_cast<unsigned int>(Rs.word) & 0x80000000);
    set_register(_registers(Rd.word),
                 (static_cast<uint32_t>((Rs.word >> (val)) | sign)));
}

/**
 * @brief if I[0]: Subtract contents of Rn to contents of Rs. Place result in
 * Rd. else: Subtract 3-bit immediate value to contents of Rs. Place result in
 * Rd.
 */
void Arm7tdmi::SUB_t(Arm7tdmi::_instruction ins)
{
    ...
    set_register(_registersRd.word,
                 (static_cast<uint32_t>(Rs.word - Rn_Offset3)));
}
...
void Arm7tdmi::ADC_t(Arm7tdmi::_instruction instruction)
{
    ...
    rn = registers[_registers((instruction.halfword_lo >> 4) & 0xF)]; // Rn is
                                                                      // Rs in
                                                                      // THUMB
    rd.word = (rd.word + rn.word) + registers[CPSR].C; // Not sure if cpu should
 ...
 */
void Arm7tdmi::ADD_t(Arm7tdmi::_instruction ins) // NEED TO IMPLEMENT LATER IN A
                                                 // MEGA ADD
{
    _register_type Rd;  // Destination Register
    _register_type Rs;  // Source Register
    ...
    if (ins.halfword_lo) // FIXME what is this?
        Rd = registers[_registers(ins.halfword_lo & 0x7)];
    Rs = registers[_registers((ins.halfword_lo >> 3) & 0x7)];
    ...
    // set_register(_registersRd.word, (Rs.word + Rn_Offset3));
...
void Arm7tdmi::AND_t(Arm7tdmi::_instruction instruction)
{
    ...
    /*Calcolo dell'operand 2 che non ho voglia di fare perchè mi hai detto di
     * fare prima la thumb xd*/
}
