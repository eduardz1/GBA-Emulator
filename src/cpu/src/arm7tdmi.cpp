 ...
 * 4a - Forces the PC to fetch the next instruction from the relevant exception
 * vector \n Entering from THUMB state forces a switch into ARM state when PC is
 * loeaded with the exception vector address
 *
 * On completion: \n
...
{
    /*
     * offset value is 4 or 8 (bytes) depending on the exception when entering
     * from ARM state, in THUMB the exception handler is responsible for
     * choosing an offset such that the program resumes from the correct place
     * on return from an exception
     */
    uint8_t offset;
    ...
    if (get_mode() == ARM_MODE)
    {
        // 1a
        registers[link].word =
            registers[PC + 4].word; // or +8, still gotta handle that
    }
    else // THUMB_MODE
    {
        // 1a
    ...
    // 3a - switch(exception){}
    // 4a - registers[PC] = exceptions[EXC]; with "exceptions" being an array of
    // function pointers
}
 ...
 */
void Arm7tdmi::set_condition_code_flags(int32_t Rd,
                                        int32_t Rn,
                                        int32_t op2,
                                        bool overflowable)
{
    ...
    if ((Rn > 0 && op2 > 0 && Rd <= 0) ||
        (Rn < 0 && op2 < 0 && Rd >= 0)) // if true sets overflow flag
        registers[get_register(CPSR)].V = 1;
    else
        registers[get_register(CPSR)].V = 0;

    if ((Rn > 0 || (op2 > 0 && Rd <= 0)) ||
        (Rn < 0 && op2 < 0 && Rd >= 0)) // if true sets carry flag
...
void Arm7tdmi::set_mode(enum _mode mode)
{
    ...
    switch (mode)
    {
    case ARM_MODE: registers[R15].word &= 0xFFFFFFFC; break;

    case THUMB_MODE: registers[R15].word &= 0xFFFFFFFE; break;
    }
}

/*Not sure if the current mode I am in has to be taken into account when
  fetching the new mode Aka, should the mode be got in the SPSR_svc register if
  i am in the supervisor register? be got */
...
int32_t Arm7tdmi::get_ALU_op2(_shift type, _instruction ins)
{
    ...
    else // No immediate operand(bit[25]/I flag unset)
    {
        ...
        case LL:
            ...
            break;
        case LR:
            op2 = static_cast<int32_t>(uint32_t(op2) >> shift);
            if (ins.word & 0x00100000)
                registers[get_register(CPSR)].C = (op2 >> (shift - 1)) & 1;
            break;
        case AR:
...
{
    registers[reg].word = static_cast<int32_t>(val);
}

// Passing an entire bus is too memory heavy, we need to implement it in another
// way
 ...
 * @param ins Instruction to be decoded and executed. It can be decoded either
 * in ARM mode or in THUMB mode
 */
void Arm7tdmi::decode_execute(Arm7tdmi::_instruction ins)
{
    if (get_mode() == THUMB_MODE)
        (this->*THUMB_isa[ins.word])(
            ins); // check std::invoke syntax for a prettier solution
    else
        decode_executeARM32(ins);
}

/**
 * @brief saves every possible value of a 16 bit instruction in an array of
 * function pointers to the instruction
 *
 */
void Arm7tdmi::build_THUMB_isa()
{
    for (_instruction i = {}; i.word < 65536; i.word++)
        THUMB_isa[i.word] = decode_THUMB(i);
}

/**
 * @brief decodes a THUMB instruction and returns the function pointer to the
 * corresponding function
 *
 * @param ins instruction to be decoded
 * @return Arm7tdmi::instruction_ptr pointer to the function that implements the
 * instruction
 */
Arm7tdmi::instruction_ptr Arm7tdmi::decode_THUMB(Arm7tdmi::_instruction ins)
{
    ...
    switch (hi_byte >> 4) // isolate higher 4 bits (bits[12:15])
    {
    case 0x0: {
        ...
        case 0x05:
        case 0x06:
        case 0x07: return &Arm7tdmi::LSL_t; // 0000 0xxx
        case 0x08:
        case 0x09:
        case 0x0C:
        case 0x0D:
        ...
        case 0x0B:
        case 0x0E:
        case 0x0F: return &Arm7tdmi::LSR_t; // 0000 1xxx
        }
    }
    case 0x1: {
        switch (hi_byte) // bits[8:15]
        ...
        case 0x15:
        case 0x16:
        case 0x17: return &Arm7tdmi::ASR_t; // 0001 0xxx
        case 0x18:
        case 0x19:
        case 0x1C:
        case 0x1D: return &Arm7tdmi::ADD_t; // 0001 1x0x
        case 0x1A:
        case 0x1B:
        case 0x1E:
        case 0x1F: return &Arm7tdmi::SUB_t; // 0001 1x1x
        }
    }
    case 0x2: {
        switch (hi_byte)
        ...
        case 0x25:
        case 0x26:
        case 0x27: return &Arm7tdmi::MOV_t; // 0010 0xxx
        case 0x28:
        case 0x29:
        case 0x2C:
        case 0x2D:
        ...
        case 0x2B:
        case 0x2E:
        case 0x2F: return &Arm7tdmi::CMP_t; // 0010 1xxx
        }
    }
    case 0x3: {
        switch (hi_byte)
        ...
        case 0x25:
        case 0x26:
        case 0x27: return &Arm7tdmi::ADD_t; // 0011 0xxx
        case 0x28:
        case 0x29:
        case 0x2C:
        case 0x2D:
        ...
        case 0x2B:
        case 0x2E:
        case 0x2F: return &Arm7tdmi::SUB_t; // 0011 1xxx
        }
    }
    case 0x4: {
        switch ((hi_byte & 0x0C))
    ...
    case 0x5: {
        ...
        case 0x6:
            return &Arm7tdmi::LDRB_t; // Load/Store Sign-Extended Byte/Halfword
        case 0x1: return &Arm7tdmi::STRH_t;
        case 0x3: return &Arm7tdmi::LDRH_t;
        case 0x5:
            return &Arm7tdmi::LDRSB_t; // technically should be LDSB but it
                                       // seems equivalent to ARM ::= LDSRB
        case 0x7: return &Arm7tdmi::LDRSH_t; // same as above
        }
    }
    case 0x6:
    case 0x7: {                 // Load/Store with Immediate Offset
    ...
    case 0xB: {
        ...
        case 0xB1:
            return &Arm7tdmi::SUB_t;
            // Push/Pop Registers
        case 0xB4:
        case 0xB5: return &Arm7tdmi::PUSH_t;
        case 0xBA:
        case 0xBB: return &Arm7tdmi::POP_t;
...
void Arm7tdmi::decode_executeARM32(Arm7tdmi::_instruction ins)
{
    ...
    case 0x0: {
        ...
        case 0x9:
            ...
            { // Multiply
            case 0x0:
            case 0x1: MUL_a(ins); break;
            case 0x2:
            case 0x3: MLA_a(ins); break; // Multiply Long
            case 0x8:
            case 0x9: UMULL_a(ins); break;
            case 0xA:
            case 0xB: UMLAL_a(ins); break;
            case 0xC:
            case 0xD: SMULL_a(ins); break;
            case 0xE:
            case 0xF: SMLAL_a(ins); break; // Single Data Swap
    ...
    case 0x2: {
        ...
        {
        case 0x20:
        case 0x21: B_a(ins); break;   // xx1x xxx0 U xx1x xxx1
