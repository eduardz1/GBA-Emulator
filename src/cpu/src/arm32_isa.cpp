...
/*TODO: figuring out a way to pass the bus controller/ram to a method so that
 * it can load/store data*/
...
void Arm7tdmi::ADD_a(Arm7tdmi::_instruction ins)
{
    ...
    Rd = Rn.word + op2;
    set_register(_registers(ins.Rd), static_cast<uint32_t>(Rd));
...
void Arm7tdmi::MUL_a(Arm7tdmi::_instruction ins)
{
    ...
    set_register(_registers(ins.Rn),
                 static_cast<uint32_t>(Rd)); // Rd is in position Rn
    if (ins.word & 0x00100000)               // flag S set
 ...
 * flush and refill from the address specified by Rn. This instruction also
 * permits the instruction set to be exchanged. When the instruction is
 * executed, the value of Rn[0] determines whether the instruction stream will
 * be decoded as ARM or THUMB instructions.
 *
 * @param ins instruction
 */
 ...
 * be 2 words (8 bytes) ahead of the current instruction. Branches beyond +/-
 * 32Mbytes must use an offset or absolute destination which has been
 * previously loaded into a register. In this case the PC should be manually
 * saved in R14 if a Branch with Link type operation is required.
 *
 * @param ins instruction
 */
 ...
 * R14[1:0] are always cleared. To return from a routine called by Branch with
 * Link use MOV PC,R14 if the link register is still valid or   LDM Rn!,{..PC}
 * if the link register has been saved onto a stack pointed to by Rn.
 *
 * @param ins instruction
 */
 ...
 * @details
 *  The instruction is only executed if the condition is true.
 *  The single data transfer instructions are used to load or store single
 * bytes or words of data. The memory address used in the transfer is
 * calculated by adding an offset to or subtracting an offset from a base
 * register. The result of this calculation may be written back into the base
 * register if auto-indexing is required.
 *
 * @param ins instruction
 */
void Arm7tdmi::LDR_a(Arm7tdmi::_instruction ins)
{
    ...
    _register_type base_register = registers[get_register(_registersins.Rn)];
